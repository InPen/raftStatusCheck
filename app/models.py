...
class Case(db.Model):
    ...
    # minimal fields for now – we’ll expand later
    client_name = db.Column(db.String(120), nullable=False)
    raft_case_number = db.Column(db.String(64), nullable=False, index=True)
    client_last_name = db.Column(db.String(120), nullable=False)

    status = db.Column(db.String(80))
    submitted_date = db.Column(db.String(80))   # keep as string for now, we can normalize later
